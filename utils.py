        ...
        for k, v in kwargs.items():
            ...
            if isinstance(v, torch.Tensor):
                if torch.numel(v) == 1:
                    ...
                    self.writer.add_histogram(head + "/" + k, v.reshape(-1), self.step if step is None else step)
...
def save_snapshot(snapshot_dir, step, model_without_ddp):
    snapshot_dir = Path(snapshot_dir)
    step_name = str(step)
    checkpoint_path = snapshot_dir / ('%s.pth' % step_name)
    to_save = {
        'model': model_without_ddp.state_dict()
    }
    save_on_master(to_save, checkpoint_path)

def load_snapshot(snapshot_dir, step, model_without_ddp):
    snapshot_dir = Path(snapshot_dir)
    step_name = str(step)
    checkpoint_path = snapshot_dir / ('%s.pth' % step_name)
...
def build_model(args):
    if args.model.startswith("convnext"):
        ...
        model = MLP(args.input_size, args.num_layers, args.dropout, args.nb_classes)
    else:
        model = create_model(
            args.model, 
            pretrained=False, 
